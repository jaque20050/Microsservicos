...
public class PessoaController {
    ...
    @PutMapping(value="/{id}")
    public ResponseEntity<PessoaModeloResponseDetalhes> atualizarPessoa(@PathVariable Integer id,
        @Valid @RequestBody Pessoa pessoa) {
        ModelMapper mapper = new ModelMapper();
        PessoaDto dto = mapper.map(pessoa, PessoaDto.class);
        dto = service.atualizarPessoa(id, dto);

        return new ResponseEntity<>(mapper.map(dto, PessoaModeloResponseDetalhes.class), HttpStatus.OK);
    }
